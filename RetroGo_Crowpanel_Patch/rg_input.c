...
#ifdef RG_GPIO_TOUCH_CS
#include <driver/spi_master.h>
static spi_device_handle_t touch_spi = NULL;

static bool read_touch(int *x, int *y) {
    if (!touch_spi) return false;
    uint8_t tx_data_x[3] = {0xD0, 0x00, 0x00};
    uint8_t rx_data_x[3] = {0};
    spi_transaction_t t_x = {
        .length = 24,
        .tx_buffer = tx_data_x,
        .rx_buffer = rx_data_x
    };
    spi_device_polling_transmit(touch_spi, &t_x);
    int raw_x = ((rx_data_x[1] << 8) | rx_data_x[2]) >> 3;

    uint8_t tx_data_y[3] = {0x90, 0x00, 0x00};
    uint8_t rx_data_y[3] = {0};
    spi_transaction_t t_y = {
        .length = 24,
        .tx_buffer = tx_data_y,
        .rx_buffer = rx_data_y
    };
    spi_device_polling_transmit(touch_spi, &t_y);
    int raw_y = ((rx_data_y[1] << 8) | rx_data_y[2]) >> 3;

    if (raw_x < 300 || raw_x > 3800 || raw_y < 300 || raw_y > 3800) {
        return false;
    }

    *x = (raw_x - 275) * 240 / (3620 - 275);
    *y = (raw_y - 264) * 320 / (3532 - 264);
...
#if defined(RG_GPIO_TOUCH_CS)
    int tx, ty;
    if (read_touch(&tx, &ty)) {
        if (ty > 160) {
            if (tx < 120) {
                if (tx < 40) state |= RG_KEY_LEFT;
                else if (tx > 80) state |= RG_KEY_RIGHT;
                else if (ty < 240) state |= RG_KEY_UP;
                else state |= RG_KEY_DOWN;
            } else {
                if (tx < 180) state |= RG_KEY_B;
                else state |= RG_KEY_A;
            }
        } else if (ty < 40) {
            if (tx < 80) state |= RG_KEY_SELECT;
            else if (tx > 160) state |= RG_KEY_START;
            else state |= RG_KEY_MENU;
        }
    }
#endif
...
void rg_input_init(void)
{
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .mode = 0,
        .spics_io_num = RG_GPIO_TOUCH_CS,
        .queue_size = 1
    };
    spi_bus_add_device(RG_SCREEN_HOST, &devcfg, &touch_spi);
#endif
