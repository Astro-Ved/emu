...
static byte touchCheck()
{
  uint16_t t_x = 0, t_y = 0;
  bool pressed = screen.getTouch(&t_x, &t_y);
  byte buttonVals = 0;

  if (pressed) {
    // Left side D-pad
    if (t_x < 80 && t_y > 240) {
      buttonVals |= (1 << P1_Left);
    } else if (t_x > 80 && t_x < 160 && t_y > 240) {
      buttonVals |= (1 << P1_Right);
    } else if (t_x > 40 && t_x < 120 && t_y > 160 && t_y < 240) {
      buttonVals |= (1 << P1_Top);
    } else if (t_x > 40 && t_x < 120 && t_y > 280) {
      buttonVals |= (1 << P1_Bottom);
    }

    // Right side A/B
    if (t_x > 160 && t_x < 200 && t_y > 240) {
      buttonVals |= (1 << P2_Left); // B button
    } else if (t_x > 200 && t_y > 240) {
      buttonVals |= (1 << P2_Right); // A button
    }
  }
  return buttonVals;
}
...
#if defined(ESP32) && defined(CROWPANEL)
  if ((buttons & UP_BUTTON) && (buttons & A_BUTTON) && (buttons & B_BUTTON)) {
