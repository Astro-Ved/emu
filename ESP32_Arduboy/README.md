...
## Elecrow Crowpanel Basic 2.4" Support
This port includes support for the Elecrow Crowpanel Basic 2.4 inch display using touch controls to emulate the Arduboy buttons.
To use it:
1. Define `CROWPANEL` in `src/Arduboy2Core.h`.
2. Copy the content of `Crowpanel_User_Setup.h` to your `TFT_eSPI/User_Setup.h` (or configure your PlatformIO `build_flags` accordingly) to set up the correct pins and drivers.
