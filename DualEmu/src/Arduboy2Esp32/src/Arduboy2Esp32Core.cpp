...
{
  // Hardware OLED boot removed. Framebuffer will be rendered by TFT_eSPI in the main sketch.
}
...
// Expose an external weak hook so that the main sketch can grab the frame.
__attribute__((weak)) void drawArduboyFrameHook(const uint8_t* buffer) {}

void Arduboy2Core::paintScreen(const uint8_t *image)
{
  drawArduboyFrameHook(image);
}
